	...
	<div class="content">
		...
		<div class="visitorContainer">
			...
			<mu-row v-if="hasVisitor">
				<mu-col span="2" v-for="(v,index) in visitor" :key="index">
					<div class="grid-cell visitor"><img :src="v.avatar" @click="goFriendData(v.uid)" /></div>
				</mu-col>
			</mu-row>
									...
									<div v-for="(s,index) in stolens" class="stealerList">
										<div class="stealerLeft">
											<div class="stealerTitle">
												<span class="stealerTextName">{{s.thief.nickname}}</span> <span class="stealerText">偷取了你的珍珠</span><span class="stealerText">{{s.stolen_reward}}</span>
											</div>
							...
							<mu-container ref="container" class="demo-loadmore-content" style="padding: 0">
								<mu-load-more :refreshing="refreshingSteal" :loading="loadingSteal" @load="loadSteal">
									<div v-for="(s,index) in steals" class="stealerList">
										<div class="stealerLeft">
											<div class="stealerTitle">
												<span class="stealerText">你偷取了</span><span class='stealerTextName'>{{s.user.nickname}}</span><span class="stealerText">的珍珠</span><span class="stealerText">{{s.stolen_reward}}</span>
											</div>
						...
						<div class="panel panel-default">
							<div class="panel-heading" style="background: #FAFAFA">
								<a data-toggle="collapse" data-parent="#accordion" :href="m.href">
									<h4 class="panel-title"> <span class="glyphicon glyphicon-chevron-down" style="color: #777" v-show="m.hasMsg"></span> {{m.content}}</h4></a>
							</div>
...
<style scoped>
	...
	div.mu-tabs.mu-tabs-full-width {
